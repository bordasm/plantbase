...
> A "mivel" (verziók, eszközlista, séma) a `stack.md`-ben; itt a STRUKTÚRA és a kulcsdöntések.
...
1. **Framework-agnostic core.** A `packages/core` nem ismeri a belépési pontokat (CLI/API/web). Új felület = új app, nem újraírás. (Mastra később a core köré.)
...
5. **Lokális DB.** docker-compose Postgres, Docker Desktop futtatja. Helyben dolgozunk, nincs felhő-DB.
